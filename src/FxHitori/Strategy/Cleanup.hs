...
handleBlack board cell = map buildMove targets
  where 
    targets = board ^.. adjCellsTo cell . filtered ( \ (Cell _ _ st) -> isNothing st)
...
cleanupWhiteMoves board = board ^.. whiteCells >>= handleWhite board

cleanupBlackMoves :: Board -> [(Move, Explanation)]
cleanupBlackMoves board = board ^.. blackCells >>= handleBlack board
